...
class AzureLogAnalyticsBackend(SingleTextQueryBackend):
    ...
    def generate(self, sigmaparser):
        ...
        if "keywords" in detection.keys():
            # if detection["condition"] == "keywords":
            #     query = super().generate(sigmaparser).replace("| where","| search")
            # elif len(detection["keywords"]) == 1:
            #     keyword = detection["keywords"][0]
            #     query = query.replace(f") and \"{keyword}\")",f")) | search \"{keyword}\"")
            return query
